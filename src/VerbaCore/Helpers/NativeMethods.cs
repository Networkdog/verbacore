...
internal static class NativeMethods
{
    ...
    // Thread message loop — required so the low-level keyboard hook can run on a
    // dedicated thread. Pumping messages here (instead of on the UI thread)
    // guarantees the hook callback is never blocked by slow UI work.
